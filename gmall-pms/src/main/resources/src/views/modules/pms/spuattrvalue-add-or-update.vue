    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/pms/spuattrvalue/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.spuAttrValue.spuId
                this.dataForm.attrId = data.spuAttrValue.attrId
                this.dataForm.attrName = data.spuAttrValue.attrName
                this.dataForm.attrValue = data.spuAttrValue.attrValue
                this.dataForm.sort = data.spuAttrValue.sort
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/pms/spuattrvalue/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
