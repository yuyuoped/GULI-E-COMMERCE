    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/pms/skuattrvalue/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.skuAttrValue.skuId
                this.dataForm.attrId = data.skuAttrValue.attrId
                this.dataForm.attrName = data.skuAttrValue.attrName
                this.dataForm.attrValue = data.skuAttrValue.attrValue
                this.dataForm.sort = data.skuAttrValue.sort
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/pms/skuattrvalue/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
