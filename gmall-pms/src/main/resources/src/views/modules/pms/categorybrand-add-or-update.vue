    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/pms/categorybrand/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.brandId = data.categoryBrand.brandId
                this.dataForm.categoryId = data.categoryBrand.categoryId
                this.dataForm.brandName = data.categoryBrand.brandName
                this.dataForm.categoryName = data.categoryBrand.categoryName
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/pms/categorybrand/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
