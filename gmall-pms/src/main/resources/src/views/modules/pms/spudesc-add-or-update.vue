    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.spuId) {
            this.$http({
              url: this.$http.adornUrl(`/pms/spudesc/info/${this.dataForm.spuId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.decript = data.spuDesc.decript
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/pms/spudesc/${!this.dataForm.spuId ? 'save' : 'update'}`),
              method: 'post',
