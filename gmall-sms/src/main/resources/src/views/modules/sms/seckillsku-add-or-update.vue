    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sms/seckillsku/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.promotionId = data.seckillSku.promotionId
                this.dataForm.promotionSessionId = data.seckillSku.promotionSessionId
                this.dataForm.skuId = data.seckillSku.skuId
                this.dataForm.seckillPrice = data.seckillSku.seckillPrice
                this.dataForm.seckillCount = data.seckillSku.seckillCount
                this.dataForm.seckillLimit = data.seckillSku.seckillLimit
                this.dataForm.seckillSort = data.seckillSku.seckillSort
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sms/seckillsku/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
