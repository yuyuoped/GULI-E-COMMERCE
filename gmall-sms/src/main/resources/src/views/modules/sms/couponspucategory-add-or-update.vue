    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sms/couponspucategory/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.couponId = data.couponSpuCategory.couponId
                this.dataForm.categoryId = data.couponSpuCategory.categoryId
                this.dataForm.categoryName = data.couponSpuCategory.categoryName
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sms/couponspucategory/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
