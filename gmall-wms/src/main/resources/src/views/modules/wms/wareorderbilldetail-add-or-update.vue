    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/wms/wareorderbilldetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.wareOrderBillDetail.skuId
                this.dataForm.skuName = data.wareOrderBillDetail.skuName
                this.dataForm.skuNum = data.wareOrderBillDetail.skuNum
                this.dataForm.taskId = data.wareOrderBillDetail.taskId
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/wms/wareorderbilldetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
